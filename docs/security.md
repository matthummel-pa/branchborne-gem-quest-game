...
- **The Save screen.** It does not collect an email or a password. A downloaded file can include a PIN hash and a salt. The raw PIN is not written into the file, `localStorage`, or the commit log.
