...
-- Create the table when this project does not already have it. Later statements
-- add quest columns. This does not touch auth allowlist triggers.
...
create table if not exists public.branchborne_saves (
  ...
  constraint branchborne_saves_high_score_check check (high_score >= 0),
  constraint branchborne_saves_trophies_check check (jsonb_typeof(trophies) = 'array'),
  constraint branchborne_saves_items_check check (jsonb_typeof(items) = 'array')
);
